...
impl<'a> ParserImpl<'a> {
    ...
    pub(crate) fn parse_variable_statement(
        ...
        if stmt_ctx.is_single_statement() && decl.kind.is_lexical() {
            self.error(diagnostics::LexicalDeclarationSingleStatement(
                decl.span,
            ));
        }
    ...
    fn parse_for_statement(&mut self) -> Result<Statement<'a>> {
        ...
        self.parse_for_loop(
            span,
            Some(ForStatementInit::Expression(init_expression)),
            r#await,
        )
    }
    ...
    fn parse_using_declaration_for_statement(
        ...
        if matches!(self.cur_kind(), Kind::In) {
            if using_decl.is_await {
                ...
                self.error(
                    diagnostics::UsingDeclarationNotAllowedInForInStatement(
                        using_decl.span,
                    ),
                );
            }
    ...
    fn parse_return_statement(&mut self) -> Result<Statement<'a>> {
        ...
        let argument =
            if self.eat(Kind::Semicolon) || self.can_insert_semicolon() {
                ...
                let expr = self
                    .with_context(Context::In, ParserImpl::parse_expression)?;
                self.asi()?;
                Some(expr)
            };
        if !self.ctx.has_return() {
            self.error(diagnostics::ReturnStatementOnlyInFunctionBody(
