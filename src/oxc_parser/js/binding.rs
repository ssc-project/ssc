...
impl<'a> ParserImpl<'a> {
    ...
    pub(super) fn parse_binding_pattern_kind(
        &mut self,
    ) -> Result<BindingPatternKind<'a>> {
        match self.cur_kind() {
    ...
    fn parse_binding_pattern_identifier(
        &mut self,
    ) -> Result<BindingPatternKind<'a>> {
    ...
    pub(super) fn parse_rest_element(
        &mut self,
    ) -> Result<Box<'a, BindingRestElement<'a>>> {
        let span = self.start_span();
        ...
        let argument = self.with_context(Context::In, |p| {
        ...
        if self.at(Kind::Comma) {
            if self.peek_at(Kind::RBrack) {
                self.error(diagnostics::BindingRestElementTrailingComma(
                    self.cur_token().span(),
                ));
    ...
    pub(super) fn parse_binding_property(
        &mut self,
    ) -> Result<BindingProperty<'a>> {
        let span = self.start_span();
        ...
        let value = if is_binding_identifier && !self.at(Kind::Colon) {
            ...
            if let PropertyKey::Identifier(ident) = &key {
                ...
                self.with_context(Context::In, |p| {
