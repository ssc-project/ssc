...
use oxc_syntax::{
    operator::BinaryOperator, precedence::Precedence, BigintBase, NumberBase,
};
...
use super::{
    ...
    operator::{
        kind_to_precedence, map_assignment_operator, map_binary_operator,
...
impl<'a> ParserImpl<'a> {
    ...
    pub(crate) fn parse_label_identifier(
        &mut self,
    ) -> Result<LabelIdentifier<'a>> {
        if !self
            .cur_kind()
    ...
    pub(crate) fn parse_identifier_name(
        &mut self,
    ...
    pub(crate) fn parse_keyword_identifier(
        &mut self,
        kind: Kind,
    ) -> IdentifierName<'a> {
    ...
    #[inline]
    pub(crate) fn parse_identifier_kind(
        &mut self,
        kind: Kind,
    ) -> (Span, Atom<'a>) {
        let span = self.start_span();
    ...
    fn parse_parenthesized_expression(
        ...
        if expressions.is_empty() {
            return Err(
                diagnostics::EmptyParenthesizedExpression(paren_span).into()
            );
        }
        ...
        match self.cur_kind() {
            ...
            kind if kind.is_number() => {
                if self.cur_src().ends_with('n') {
                    self.parse_literal_bigint().map(|literal| {
                        self.ast.literal_bigint_expression(literal)
                    })
                } else {
                    self.parse_literal_number().map(|literal| {
                        self.ast.literal_number_expression(literal)
                    })
                }
            }
            _ => Err(self.unexpected()),
    ...
    fn parse_template_literal(
        ...
        match self.cur_kind() {
            ...
            Kind::TemplateHead => {
                ...
                let expr =
                    self.with_context(Context::In, Self::parse_expression)?;
    ...
    fn parse_template_literal_expression(
        ...
        self.parse_template_literal(tagged).map(|template_literal| {
            self.ast.template_literal_expression(template_literal)
        })
    }
    ...
    fn parse_tagged_template(
        ...
        Ok(self.ast.tagged_template_expression(
            span,
            lhs,
            quasi,
            type_parameters,
    ...
    pub(crate) fn parse_template_element(
        &mut self,
        tagged: bool,
    ) -> TemplateElement<'a> {
        let span = self.start_span();
        ...
        let tail = matches!(
            cur_kind,
            Kind::TemplateTail | Kind::NoSubstitutionTemplate
        );
        TemplateElement {
            span,
            tail,
    ...
    pub(crate) fn parse_lhs_expression(&mut self) -> Result<Expression<'a>> {
        ...
        let lhs =
            self.parse_call_expression(span, lhs, &mut in_optional_chain)?;
        if in_optional_chain {
            let span = self.end_span(span);
    ...
    fn map_to_chain_expression(
        ...
        match expr {
            Expression::MemberExpression(result) => self
                .ast
                .chain_expression(span, ChainElement::MemberExpression(result)),
            Expression::CallExpression(result) => self
                .ast
                .chain_expression(span, ChainElement::CallExpression(result)),
            expr => expr,
        }
    }
    ...
    fn parse_member_expression_base(
        ...
        let span = self.start_span();
        self.parse_primary_expression().and_then(|lhs| {
            self.parse_member_expression_rhs(span, lhs, in_optional_chain)
        })
    }
    ...
    fn parse_member_expression_rhs(
        ...
        loop {
            lhs = match self.cur_kind() {
                ...
                Kind::Dot => {
                    self.parse_static_member_expression(lhs_span, lhs, false)?
                }
                Kind::QuestionDot => {
                    *in_optional_chain = true;
                    match self.peek_kind() {
                        ...
                        Kind::PrivateIdentifier => self
                            .parse_static_member_expression(
                                lhs_span, lhs, true,
                            )?,
                        kind if kind.is_identifier_name() => self
                            .parse_static_member_expression(
                                lhs_span, lhs, true,
                            )?,
                        _ => break,
                        ...
                        && self.ts_enabled() =>
                {
                    self.bump_any();
                    self.ast
                        .ts_non_null_expression(self.end_span(lhs_span), lhs)
                }
                kind if kind.is_template_start_of_tagged_template() => {
                    let (expr, type_parameters) =
                        if let Expression::TSInstantiationExpression(
                            instantiation_expr,
                        ) = lhs
                        {
                ...
                Kind::LAngle | Kind::ShiftLeft if self.ts_enabled() => {
                    if let Some(arguments) =
                        self.parse_ts_type_arguments_in_expression()
                    {
                        lhs = Expression::TSInstantiationExpression(
                            self.ast.alloc(TSInstantiationExpression {
    ...
    fn parse_static_member_expression(
        ...
        if self.cur_kind() == Kind::PrivateIdentifier {
            ...
            Ok(self.ast.static_member_expression(
                self.end_span(lhs_span),
                lhs,
                ident,
                optional,
    ...
    fn parse_computed_member_expression(
        ...
        Ok(self.ast.computed_member_expression(
            self.end_span(lhs_span),
            lhs,
            property,
            optional,
    ...
    fn parse_new_expression(&mut self) -> Result<Expression<'a>> {
        ...
        if let Expression::TSInstantiationExpression(instantiation_expr) =
            callee
        {
    ...
    fn parse_call_expression(
        ...
        loop {
            ...
            lhs = self.parse_member_expression_rhs(
                lhs_span,
                lhs,
                in_optional_chain,
            )?;
            ...
            *in_optional_chain =
                if optional_call { true } else { *in_optional_chain };
    ...
    fn parse_call_arguments(
        ...
        //   AssignmentExpression[+In, ?Yield, ?Await]
        let call_arguments =
            self.with_context(Context::In, CallArguments::parse)?;
    ...
    fn parse_binary_or_logical_expression_base(
        ...
        let lhs = if self.ctx.has_in() && self.at(Kind::PrivateIdentifier) {
            ...
            Expression::PrivateInExpression(self.ast.alloc(
                PrivateInExpression {
                    span: self.end_span(lhs_span),
                    left,
        ...
        self.parse_binary_or_logical_expression_recursive(
            lhs_span,
            lhs,
            lhs_precedence,
        )
    ...
    fn parse_binary_or_logical_expression_recursive(
        ...
        loop {
            ...
            let rhs =
                self.parse_binary_or_logical_expression_base(left_precedence)?;
    ...
    fn parse_conditional_expression(&mut self) -> Result<Expression<'a>> {
        let span = self.start_span();
        let lhs =
            self.parse_binary_or_logical_expression_base(Precedence::lowest())?;
        ...
        Ok(self.ast.conditional_expression(
            self.end_span(span),
            lhs,
            consequent,
            alternate,
    ...
    pub(crate) fn parse_assignment_expression_base(
        &mut self,
    ) -> Result<Expression<'a>> {
            ...
            IsParenthesizedArrowFunction::Maybe => {
                ...
                if !self.state.not_parenthesized_arrow.contains(&pos) {
                    if let Ok((
                        ...
                        ParserImpl::parse_parenthesized_arrow_function_head,
                    ) {
            ...
            && self.nth_at(2, Kind::Arrow)
        {
            ...
            if arrow_token.is_on_new_line {
                self.error(diagnostics::NoLineBreakIsAllowedBeforeArrow(
                    arrow_token.span(),
                ));
            }
    ...
    fn parse_assignment_expression_recursive(
        ...
        Ok(self.ast.assignment_expression(
            self.end_span(span),
            operator,
            left,
            right,
    ...
    fn parse_await_expression(
        ...
        if !has_await {
            self.error(diagnostics::AwaitExpression(Span::new(
                span.start,
                span.start + 5,
            )));
        }
    ...
    pub(crate) fn parse_decorator(&mut self) -> Result<Decorator<'a>> {
        ...
        self.bump_any(); // bump @
        let expr =
            self.with_context(Context::Decorator, Self::parse_lhs_expression)?;
    ...
    fn is_await_expression(&mut self) -> bool {
        if self.at(Kind::Await) {
            ...
            return peek_token.kind.is_after_await_or_yield()
