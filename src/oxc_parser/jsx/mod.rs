...
use crate::oxc_parser::{diagnostics, lexer::Kind, Context, ParserImpl};
...
impl<'a> ParserImpl<'a> {
    ...
    /// <>
    fn parse_jsx_opening_fragment(
        &mut self,
        span: Span,
    ) -> Result<JSXOpeningFragment> {
    ...
    fn parse_jsx_member_expression(
        ...
        let mut span = span;
        let mut object =
            JSXMemberExpressionObject::Identifier(self.ast.alloc(object));
        ...
        if let Some(property) = property {
            return Ok(self.ast.jsx_member_expression(
                self.end_span(span),
                object,
                property,
            ));
    ...
    fn parse_jsx_child(&mut self) -> Result<Option<JSXChild<'a>>> {
        match self.cur_kind() {
            ...
            // <ident open element
            Kind::LAngle
                if self.peek_at(Kind::Ident)
                    || self.peek_kind().is_all_keyword() =>
            {
    ...
    fn parse_jsx_assignment_expression(&mut self) -> Result<Expression<'a>> {
        ...
        if let Ok(Expression::SequenceExpression(seq)) = &expr {
            return Err(diagnostics::JSXExpressionsMayNotUseTheCommaOperator(
                seq.span,
            )
            .into());
        }
    ...
    fn parse_jsx_spread_child(
        &mut self,
    ) -> Result<Box<'a, JSXSpreadChild<'a>>> {
        let span = self.start_span();
        self.bump_any(); // bump `{`
    ...
    fn parse_jsx_attributes(
        &mut self,
    ) -> Result<Vec<'a, JSXAttributeItem<'a>>> {
        ...
        while !matches!(
            ...
            let attribute = match self.cur_kind() {
                ...
                _ => {
                    self.parse_jsx_attribute().map(JSXAttributeItem::Attribute)
                }
            }?;
    ...
    fn parse_jsx_spread_attribute(
        &mut self,
    ) -> Result<Box<'a, JSXSpreadAttribute<'a>>> {
        let span = self.start_span();
        self.bump_any(); // bump `{`
        self.expect(Kind::Dot3)?;
    ...
    fn parse_jsx_attribute_name(&mut self) -> Result<JSXAttributeName<'a>> {
        ...
        if self.eat(Kind::Colon) {
            let property = self.parse_jsx_identifier()?;
            return Ok(JSXAttributeName::NamespacedName(
                self.ast.jsx_namespaced_name(
                    self.end_span(span),
                    identifier,
                    property,
    ...
    fn parse_jsx_attribute_value(&mut self) -> Result<JSXAttributeValue<'a>> {
        match self.cur_kind() {
            Kind::Str => self.parse_literal_string().map(|str_lit| {
                JSXAttributeValue::StringLiteral(self.ast.alloc(str_lit))
            }),
            Kind::LCurly => {
                let expr = self.parse_jsx_expression_container(
                    /* is_jsx_child */ false,
                )?;
    ...
    fn parse_jsx_identifier(&mut self) -> Result<JSXIdentifier<'a>> {
        ...
        // Currently at a valid normal Ident or Keyword, keep on lexing for `-`
        // in `<component-name />`
