...
use crate::oxc_parser::{UniquePromise, MAX_LEN};
...
impl<'a> Source<'a> {
    ...
    pub(super) fn new(
        ...
        // SAFETY: Adding `source_text.len()` to the starting pointer gives a
        // pointer at the end of `source_text`. `end` will never be
        // dereferenced, only checked for direct pointer equality with
        // `ptr` to check if at end of file.
        let end = unsafe { start.add(source_text.len()) };

        // `saturating_sub` not `wrapping_sub` so that value doesn't wrap around
        // if source is very short, and has very low memory address
    ...
    pub(super) fn whole(&self) -> &'a str {
        // SAFETY:
        // `start` and `end` are created from a `&str` in `Source::new`, so
        // `start` cannot be after `end`. `start` and `end` are by
        // definition on UTF-8 char boundaries.
        unsafe {
    ...
    pub(super) fn remaining(&self) -> &'a str {
        // SAFETY:
        // Invariant of `Source` is that `ptr` is always <= `end`, and is on a
        // UTF-8 char boundary. `end` is pointer to end of original
        // `&str`, so be definition a UTF-8 char boundary.
        unsafe {
            self.str_between_positions_unchecked(
    ...
    pub(super) fn position(&self) -> SourcePosition<'a> {
        // SAFETY: Creating a `SourcePosition` from current position of `Source`
        // is always valid, if caller has upheld safety conditions of
        // other unsafe methods of this type.
    ...
    pub(super) fn set_position(&mut self, pos: SourcePosition) {
        // `SourcePosition` always upholds the invariants of `Source`, as long
        // as it's created from this `Source`. `SourcePosition`s can
        // only be created from a `Source`. `Source::new` takes a
        // `UniquePromise`, which guarantees that it's the only `Source`
        // in existence on this thread. `Source` is not `Sync` or `Send`, so no
        // possibility another `Source` originated on another thread can
        // "jump" onto this one. This is sufficient to guarantee that
        ...
        // * The preceding checks that `pos.ptr` >= `self.start` and <
        //   `self.end`.
        // * `Source`'s invariants guarantee that `self.start` - `self.end`
        //   contains allocated memory.
        // * `Source::new` takes an immutable ref `&str`, guaranteeing that the
        //   memory `pos.ptr` addresses cannot be aliased by a `&mut` ref as
        //   long as `Source` exists.
        // * `SourcePosition` can only live as long as the `&str` underlying
        //   `Source`.
        debug_assert!(
            pos.ptr >= self.start
                && pos.ptr <= self.end
    ...
    pub(super) fn str_from_pos_to_end(&self, pos: SourcePosition) -> &'a str {
        // SAFETY: Invariants of `SourcePosition` is that it cannot be after end
    ...
    pub(super) unsafe fn str_between_positions_unchecked(
        ...
        // `Source::new` takes a `UniquePromise`, which guarantees that it's the
        // only `Source` in existence on this thread. `Source` is not
        // `Sync` or `Send`, so no possibility another
        // `Source` originated on another thread can "jump" onto this one.
        // This is sufficient to guarantee that any `SourcePosition` that
        // parser/lexer holds must be from this `Source`, therefore
        // `start.ptr` and `end.ptr` must both be within the same
        // allocation, and derived from the same original pointer.
        // Invariants of `Source` and `SourcePosition` types guarantee that both
        // are positioned on UTF-8 character boundaries. So slicing
        // source text between these 2 points will always yield a valid
        // UTF-8 string.
    ...
    pub(super) fn back(&mut self, n: usize) {
        // This assertion is essential to ensure safety of `new_pos.read()` call
        // below. Without this check, calling `back(0)` on an empty
        // `Source` would cause reading out of bounds.
        ...
        // allocation of original `&str`
        let new_pos = unsafe { self.position().sub(n) };

        // Enforce invariant that `ptr` must be positioned on a UTF-8 character
        // boundary. SAFETY: `new_ptr` is in bounds of original `&str`,
        // and `n > 0` assertion ensures not at the end, so valid to
        // read a byte. `Source`'s invariants guarantee that
        // `self.start` - `self.end` contains allocated memory.
        // `Source::new` takes an immutable ref `&str`, guaranteeing that the
    ...
    pub(super) fn next_char(&mut self) -> Option<char> {
        ...
        if byte.is_ascii() {
            ...
            // Current byte is ASCII, so incremented `ptr` must be on a UTF-8
            // character boundary.
        ...
        // Create a `Chars` iterator, get next char from it, and then update
        // `self.ptr` to match `Chars` iterator's updated pointer
        // afterwards. `Chars` iterator upholds same invariants as
        // `Source`, so its pointer is guaranteed to be valid as
        // `self.ptr`.
        let mut chars = self.remaining().chars();
        // SAFETY: We know that there's a byte to be consumed, so `chars.next()`
        // must return `Some(_)`
    ...
    unsafe fn next_byte(&mut self) -> Option<u8> {
        if self.is_eof() {
            ...
            // SAFETY: Safe to read from `ptr` as we just checked it's not out
            // of bounds
    ...
    pub(super) unsafe fn next_byte_unchecked(&mut self) -> u8 {
        ...
        // Methods of this type provide no way for `ptr` to be before `start` or
        // after `end`. Therefore always valid to read a byte from
        // `ptr`, and incrementing `ptr` cannot result in `ptr > end`.
    ...
    pub(super) fn peek_char(&self) -> Option<char> {
        ...
        let mut chars = self.remaining().chars();
        // SAFETY: We know that there's a byte to be consumed, so `chars.next()`
        // must return `Some(_)`. Could just return `chars.next()` here,
        // but making it clear to compiler that this branch
        // always returns `Some(_)` may help it optimize the caller. Compiler
        // seems to have difficulty "seeing into" `Chars` iterator and
        // making deductions.
        let c = unsafe { chars.next().unwrap_unchecked() };
        Some(c)
    }
    ...
    pub(super) unsafe fn peek_byte_unchecked(&self) -> u8 {
        // SAFETY: Caller guarantees `ptr` is before `end` (i.e. not at end of
        // file). Methods of this type provide no way to allow `ptr` to
        // be before `start`. `Source`'s invariants guarantee that
        // `self.start` - `self.end` contains allocated memory.
        // `Source::new` takes an immutable ref `&str`, guaranteeing that the
        // memory `self.ptr` addresses cannot be aliased by a `&mut` ref
        // as long as `Source` exists.
...
impl<'a> SourcePosition<'a> {
    ...
    pub(super) unsafe fn read(self) -> u8 {
        ...
        // `Source` is created from a valid `&str`, so points to allocated,
        // initialized memory. `Source` conceptually holds the source
        // text `&str`, which guarantees no mutable references
        // to the same memory can exist, as that would violate Rust's aliasing
        // rules. Pointer is "dereferenceable" by definition as a `u8`
        // is 1 byte and cannot span multiple objects. Alignment is not
        // relevant as `u8` is aligned on 1 (i.e. no alignment requirements).
    ...
    pub(super) unsafe fn read2(self) -> [u8; 2] {
        // SAFETY:
        // Caller guarantees `self` is not at no later than 2 bytes before end
        // of source text. `Source` is created from a valid `&str`, so
        // points to allocated, initialized memory.
        // `Source` conceptually holds the source text `&str`, which guarantees
        // no mutable references to the same memory can exist, as that
        // would violate Rust's aliasing rules. Pointer is
        // "dereferenceable" by definition as a `u8` is 1 byte and cannot span
        // multiple objects. Alignment is not relevant as `u8` is
