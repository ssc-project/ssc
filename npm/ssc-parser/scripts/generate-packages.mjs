...
const SSC_ROOT = resolve(fileURLToPath(import.meta.url), "../..");
const PACKAGES_ROOT = resolve(SSC_ROOT, "..");
const BINARY_ROOT = resolve(SSC_ROOT, "../../napi/parser");
const MANIFEST_PATH = resolve(SSC_ROOT, "package.json");

console.log('SSC_ROOT', SSC_ROOT);
...
function package_name(target) {
  return `@ssc-parser/binding-${target}`
}
function generateNativePackage(target) {
  const binaryName = `parser.${target}.node`;

  const packageRoot = resolve(PACKAGES_ROOT, `ssc-parser-${target}`);
...
function writeManifest() {
  const packageRoot = resolve(PACKAGES_ROOT, 'ssc-parser');
  const manifestPath = resolve(packageRoot, "package.json");
...
// NOTE: Must update npm/ssc-parser/package.json
const TARGETS = [
  "win32-x64-msvc",
  "win32-arm64-msvc",
  "linux-x64-gnu",
