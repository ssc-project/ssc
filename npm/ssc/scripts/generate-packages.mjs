...
const SSCCLI_ROOT = resolve(fileURLToPath(import.meta.url), "../..");
const PACKAGES_ROOT = resolve(SSCCLI_ROOT, "..");
const REPO_ROOT = resolve(PACKAGES_ROOT, "..");
const MANIFEST_PATH = resolve(SSCCLI_ROOT, "package.json");
...
function generateNativePackage(platform, arch) {
  const packageName = `@speedy-svelte-compiler/${platform}-${arch}`;
  const packageRoot = resolve(PACKAGES_ROOT, `ssc-${platform}-${arch}`);
  ...
  const binarySource = resolve(REPO_ROOT, `ssc-${platform}-${arch}${ext}`);
  const binaryTarget = resolve(packageRoot, `ssc${ext}`);
...
function updateWasmPackage(target) {
  const packageName = `@speedy-svelte-compiler/wasm-${target}`;
...
function writeManifest(packagePath) {
  ...
  const nativePackages = PLATFORMS.flatMap((platform) =>
    ARCHITECTURES.map((arch) => [
      `@speedy-svelte-compiler/${platform}-${arch}`,
