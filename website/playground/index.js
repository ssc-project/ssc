...
import initWasm, { Ssc, SscCodegenOptions } from "@ssc/ssc_wasm";
import { getSymbolAndReferencesSpan } from "./symbols.js";
...
class Playground {
  ...
  constructor() {
    ...
    this.currentView =
      getStringFromStorage(ACTIVE_TAB_STORAGE_KEY_CODE) || "ast";
  }

  initSsc() {
    this.ssc = new Ssc();
    this.codegenOptions = new SscCodegenOptions();

    this.runSsc(this.editor.state.doc.toString());
    this.editor.dispatch({
      effects: this.linterConf.reconfigure(this.linter()),
    });
  }
  ...
  runSsc(text) {
    ...
    this.ssc.sourceText = sourceText;
  ...
  initEditor() {
    const stateListener = EditorView.updateListener.of((view) => {
      if (view.docChanged) {
        this.runSsc(view.state.doc.toString());
        return;
      }
      if (!view.docChanged && view.selectionSet && this.currentView === "ast") {
        ...
        if (ranges.length === 1 && ranges[0].empty) {
          ...
          let targetNode = findMostInnerNodeForPosition(
            root.node,
            from,
            viewerText,
          );
  ...
  updateDiagnostics() {
    const diagnostics = (this.ssc ? this.ssc.getDiagnostics() : []).map(
      (d) => ({
        ...
        EditorState.transactionExtender.of((tr) => {
          ...
          switch (this.currentLanguage()) {
            case "json":
              ...
              break;
            case "svelte":
              if (currentLanguage == svelteLanguage) return null;
              newLanguage = svelte();
              break;
            case "rust":
  ...
  run() {
    ...
    this.ssc.run(this.codegenOptions);
  ...
  highlightEditorFromViewer(e, view) {
    if (this.currentView === "symbol") {
      ...
      this.highlightEditorRange(
        ...
        ranges.map((range) =>
          EditorSelection.range(
            convertToUtf8(this.sourceTextUtf8, range.start),
            convertToUtf8(this.sourceTextUtf8, range.end),
          ),
        ),
      );
      ...
      this.highlightEditorRange(this.editor, EditorSelection.range(start, end));
    }
  }
}
...
async function main() {
  ...
  tabBtnsBindClick((tab) => {
    if (tab === "ir-copy") {
      navigator.clipboard.writeText(playground.ssc.ir);
      return;
    }
  ...
  document.getElementById("whitespace").onchange = function () {
    ...
    playground.codegenOptions.whitespace = checked;
    playground.updateView("codegen");
  };
}
