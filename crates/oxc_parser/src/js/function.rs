...
use crate::{
    diagnostics, lexer::Kind, list::SeparatedList, Context, ParserImpl,
    StatementContext,
};
...
impl<'a> ParserImpl<'a> {
    ...
    pub(crate) fn parse_function_body(
        &mut self,
    ) -> Result<Box<'a, FunctionBody<'a>>> {
        let span = self.start_span();
        self.expect(Kind::LCurly)?;

        let (directives, statements) =
            self.with_context(Context::Return, |p| {
    ...
    pub(crate) fn parse_function(
        ...
        let ctx = self.ctx;
        self.ctx =
            self.ctx.and_in(true).and_await(r#async).and_yield(generator);
    ...
    pub(crate) fn parse_function_declaration(
        ...
        if stmt_ctx.is_single_statement() {
            if decl.r#async {
                ...
                self.error(diagnostics::GeneratorFunctionDeclaration(
                    Span::new(decl.span.start, decl.params.span.end),
                ));
            }
        }
    ...
    pub(crate) fn parse_ts_function_impl(
        ...
        let id = self.parse_function_id(func_kind, r#async, generator);
        self.parse_function(
            start_span, id, r#async, generator, func_kind, modifiers,
        )
    }
    ...
    pub(crate) fn parse_yield_expression(&mut self) -> Result<Expression<'a>> {
        ...
        let has_yield = self.ctx.has_yield();
        if !has_yield {
            self.error(diagnostics::YieldExpression(Span::new(
                span.start,
                span.start + 5,
            )));
        }
    ...
    pub(crate) fn parse_function_id(
        ...
        if kind.is_id_required() && id.is_none() {
            self.error(diagnostics::ExpectFunctionName(
                self.cur_token().span(),
            ));
        }
    ...
    pub(crate) fn is_parenthesized_arrow_function_expression(
        ...
        match self.nth_kind(offset) {
            Kind::LParen => match self.nth_kind(offset + 1) {
                // '()' is an arrow expression if followed by an '=>', a type
                // annotation or body. Otherwise, a
                // parenthesized expression with a missing inner expression
                Kind::RParen => {
                ...
                // array expression or a destructing parameter
                    ...
                    || self.nth_at(offset + 1, Kind::This) =>
                {
                    match self.nth_kind(offset + 2) {
                        ...
                        // * '(a = ': an initializer or a parenthesized
                        //   assignment expression
                        // * '(a, ': separator to next parameter or a
                        //   parenthesized sequence expression
                        // * '(a)': a single parameter OR a parenthesized
                        //   expression
    ...
    pub(crate) fn is_parenthesized_arrow_function(
        &mut self,
    ) -> IsParenthesizedArrowFunction {
        match self.cur_kind() {
            Kind::LAngle | Kind::LParen => {
                self.is_parenthesized_arrow_function_expression(false)
            }
            Kind::Async => {
                let peeked = self.peek_token();
                if !peeked.is_on_new_line
                    && matches!(peeked.kind, Kind::LAngle | Kind::LParen)
                {
    ...
    pub(crate) fn parse_parenthesized_arrow_function_head(
        ...
        if let Some(this_param) = this_param {
            // const x = (this: number) => {};
            self.error(diagnostics::TSArrowFunctionThisParameter(
                this_param.span,
            ));
        }
        ...
        if self.cur_token().is_on_new_line {
            self.error(diagnostics::LineterminatorBeforeArrow(
                self.cur_token().span(),
            ));
        }
