    ...
    fn parse_element(&mut self, p: &mut ParserImpl<'a>) -> Result<()> {
        ...
        if p.is_at_named_tuple_element() {
            if p.eat(Kind::Dot3) {
                ...
                self.elements.push(TSTupleElement::TSRestType(p.ast.alloc(
                    TSRestType {
                        span: p.end_span(span),
                        type_annotation: TSType::TSNamedTupleMember(
                            p.ast.alloc(TSNamedTupleMember {
            ...
            let element_type = p.parse_ts_type()?;
            self.elements.push(TSTupleElement::TSNamedTupleMember(
                p.ast.alloc(TSNamedTupleMember {
                    span: p.end_span(span),
                    element_type,
                    label,
        ...
        if p.eat(Kind::Dot3) {
            ...
            self.elements.push(TSTupleElement::TSRestType(p.ast.alloc(
                TSRestType { span: p.end_span(span), type_annotation },
            )));
            return Ok(());
        }
        ...
        if p.eat(Kind::Question) {
            self.elements.push(TSTupleElement::TSOptionalType(p.ast.alloc(
...
impl<'a> SeparatedList<'a> for TSImportAttributeList<'a> {
    ...
    fn parse_element(&mut self, p: &mut ParserImpl<'a>) -> Result<()> {
        ...
        let name = match p.cur_kind() {
            Kind::Str => {
                TSImportAttributeName::StringLiteral(p.parse_literal_string()?)
            }
